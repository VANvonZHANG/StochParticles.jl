...
if !isnan(d_cut_aitken)
    vline!(fig[2], [d_cut_aitken * 1e9], label="Theory cutoff κ=0.30 ($(@sprintf("%.0f", d_cut_aitken*1e9)) nm)",
           linestyle = :dash, color = :steelblue, linewidth = 1.5)
end
if !isnan(d_cut_accum)
    vline!(fig[2], [d_cut_accum * 1e9], label="Theory cutoff κ=0.61 ($(@sprintf("%.0f", d_cut_accum*1e9)) nm)",
